...
func toTokens(ctx *whisper.Context, n int) []Token {
	...
	for i := 0; i < len(result); i++ {
		result[i] = Token{
			...
			// Token times via the remap getters: real-timeline under VAD,
			// raw without it.
			Start: time.Duration(ctx.Whisper_full_get_token_t0(n, i)) * time.Millisecond * 10,
			End:   time.Duration(ctx.Whisper_full_get_token_t1(n, i)) * time.Millisecond * 10,
		}
	}
	return result
}
